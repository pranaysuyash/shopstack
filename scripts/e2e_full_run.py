...
FLOWS = [
    ...
    {
        "slug": "04_shelf_scan",
        "title": "Shelf Scan (vision OCR fresh_mart.png)",
        "steps": [
            {"kind": "dismiss"},
            {"kind": "tab", "text": "Shopping"},
            {"kind": "wait", "ms": 1500},
    ...
    {
        "slug": "05_recipe_scan",
        "title": "Recipe Scan (OCR maa_laxmi.png)",
        "steps": [
            {"kind": "dismiss"},
            {"kind": "tab", "text": "Shopping"},
            {"kind": "wait", "ms": 1500},
            {"kind": "subtab", "text": "Recipe Scan"},
            {"kind": "wait", "ms": 2000},
            {"kind": "shot", "name": "05a_recipe_open.png"},
            {"kind": "upload", "path": str(IMG_MAA_LAXMI)},
            {"kind": "wait", "ms": 15000},
            {"kind": "shot", "name": "05b_recipe_after.png", "full": True},
        ],
    },
    {
        "slug": "06_market_intel",
        "title": "Market Intel (sai_pharma.png)",
        "steps": [
            {"kind": "dismiss"},
            {"kind": "tab", "text": "Shopping"},
            {"kind": "wait", "ms": 1500},
            {"kind": "subtab", "text": "Market Intel"},
            {"kind": "wait", "ms": 2000},
            {"kind": "shot", "name": "06a_intel_open.png"},
            {"kind": "upload", "path": str(IMG_SAI_PHARMA)},
            {"kind": "wait", "ms": 15000},
            {"kind": "shot", "name": "06b_intel_after.png", "full": True},
        ],
    },
    {
        "slug": "07_recipes",
        ...
        "steps": [
            ...
            {"kind": "wait", "ms": 2500},
            {"kind": "shot", "name": "07_recipes.png", "full": True},
        ],
    },
    {
        "slug": "08_trips",
        ...
        "steps": [
            ...
            {"kind": "tab", "text": "Trips"},
            {"kind": "wait", "ms": 2500},
            {"kind": "shot", "name": "08_trips.png", "full": True},
        ],
    },
    {
        "slug": "09_photo_map",
        "title": "Pantry Photo Map (grounding fridge.png)",
        "steps": [
            {"kind": "dismiss"},
            {"kind": "tab", "text": "Pantry"},
            {"kind": "wait", "ms": 1500},
            {"kind": "subtab", "text": "Photo Map"},
            {"kind": "wait", "ms": 2000},
            {"kind": "shot", "name": "09a_photo_map.png"},
            {"kind": "upload", "path": str(IMG_FRIDGE)},
            {"kind": "wait", "ms": 15000},
            {"kind": "shot", "name": "09b_photo_map_after.png", "full": True},
        ],
    },
    {
        "slug": "10_memory",
        ...
        "steps": [
            ...
            {"kind": "wait", "ms": 2500},
            {"kind": "shot", "name": "10_memory.png", "full": True},
        ],
    },
]
...
def run_step(page, step, flow_slug, app_url):
    ...
    if kind == "wait":
        ...
        return "waited %dms" % step["ms"]
    if kind == "dismiss":
        _dismiss(page)
        return "dismissed"
    if kind == "shot":
        return "screenshot -> " + _shot(
            page, flow_slug, step["name"], step.get("full", False)
        )
    if kind == "tab":
        return "tab '%s': %s" % (
            step["text"],
            "ok" if _click_tab(page, step["text"]) else "NOT FOUND",
        )
    if kind == "subtab":
        return "subtab '%s': %s" % (
            step["text"],
            "ok" if _click_subtab(page, step["text"]) else "NOT FOUND",
        )
    if kind == "upload":
        try:
            page.set_input_files("input[type='file']", step["path"], timeout=5000)
            return "uploaded %s" % Path(step["path"]).name
        except Exception as exc:
            return "upload FAILED: %s" % exc
...
def main():
        ...
        with sync_playwright() as p:
            ...
            for flow in FLOWS:
                ...
                page.on(
                    "console",
                    lambda msg: (
                        flow_console.append(msg.text[:300])
                        if msg.type == "error"
                        else None,
                ...
                try:
                    ...
                    print("  !! CRASHED: %s" % exc)
                    try:
                        _shot(page, slug, "CRASH.png", True)
                    except Exception:
                        pass
                    summary.append(
                        {
    ...
    results = {
        ...
        "videos_dir": str(VIDEOS.relative_to(ROOT)),
    ...
    if flow_videos:
        print("Per-flow WebM videos in: %s/" % VIDEOS.relative_to(ROOT))
