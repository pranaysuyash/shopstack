...
**STATUS (2026-06-13 supersession):** This module is **archived** in
`shopstack/ui/screens/_legacy/`. The 5 functions here were flagged as
"dead" by `tests/test_screens_dead_code.py` (no Python callers in
production code). However, per `motto_v3` §7 (Supersession Rule) and
§11 (Engineering Standards):

- §11: "Do not delete overbuilt, enterprise, or speculative features
  just to simplify the current view, as that creates rework later. If
  they distract from the core product, hide them from the UI instead of
  deleting the code."
- DR-P10 (Phase 10 Permissions Handoff): This module is documented in
  `Docs/HANDOFF_PHASE10_PERMISSIONS_RESTOCK_CARD_2026-06-13.md` as
  Phase 10 #1 wiring. The functions are not speculative — they wrap the
  service layer (`shopstack/services/permissions.py`) and are intended
  to be wired into the household settings UI in Phase 10.

**Decision:** Archive (not delete). The functions remain importable
from their original path (`from shopstack.ui.screens import
add_member_screen, etc.`) for backward compatibility. Re-wiring to the
household settings UI is tracked in Phase 10 handoff.
...
**Future intent (from HANDOFF_PHASE10):**

This is the canonical implementation for the household members
panel. The functions wrap the `shopstack.services.permissions` module
which has `add_member`, `remove_member`, `change_role`, `list_members`,
`list_households_for_user`. The UI wiring was deferred to keep the
Phase 8 UI scope focused.
"""
