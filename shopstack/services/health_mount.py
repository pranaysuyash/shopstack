...
Why this exists (motto_v3 §0.10 Observability Is Delivery):
  ...
  and its key dependencies are reachable. Returning HTTP 200 from
    a generic HTTP 200 only proves the server responds. It does not
    prove the DB is queryable or that the PWA shell is wired.
...
What this checks:
  ...
  2. ``web_app`` — confirms the FastAPI app has registered routes.
...
Pattern:
  ...
  registers a FastAPI route on the native web app.
...
def _check_web_app(app: Any) -> tuple[str, str]:
    """Confirm the FastAPI app has registered routes.

    An empty route table indicates the native web-app composition
    pipeline failed before registering its user-facing surface.
    """
    try:
        routes = getattr(app, "routes", None) or []
        if not routes:
            return "fail", "no routes registered"
        return "ok", f"{len(routes)} routes"
    except Exception as exc:
        return "fail", f"{type(exc).__name__}: {exc}"


def _check_pwa_assets(app: Any) -> tuple[str, str]:
...
def mount_health_endpoint(
    ...
    *,
    ...
    """Mount the ``/health/ui`` route on the native FastAPI app.

    Args:
        app: The FastAPI application returned by ``build_fastapi_app()``.
    ...
    async def _health(request: Request) -> JSONResponse:
        ...
        s, d = _check_web_app(app)
        checks["web_app"] = {"status": s, "detail": d}
        s, d = _check_pwa_assets(app)
        checks["pwa_assets"] = {"status": s, "detail": d}

        # Overall: fail only on hard failures (database or web_app).
        # pwa_assets is best-effort and never degrades overall health.
        hard_failures = [
            name for name, c in checks.items()
            if name in ("database", "web_app") and c["status"] == "fail"
        ]
    ...
    try:
        ...
        app.add_route(path, _health, methods=["GET"])
