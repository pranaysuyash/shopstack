...
def _find_json_file(data_dir: Path | None = None) -> Path:
    ...
    candidates = sorted(d.glob("zepto_fresh_vegetables_*.json"))
    if not candidates:
        raise FileNotFoundError(
            f"No zepto_fresh_vegetables_*.json found in {d}"
        )
    return candidates[-1]
...
def normalize_record(
    ...
    raw_name = str(raw.get("item_name", "")).strip()
    ...
    sale_price = _coerce_float(raw.get("sale_price"))
    original_price_val = _coerce_float(raw.get("original_price"))
    price = sale_price if sale_price > 0 else original_price_val
    mrp = original_price_val if original_price_val > 0 and original_price_val != sale_price else sale_price
...
__all__ = ["ZeptoAdapter", "load_snapshot", "snapshot_freshness"]
