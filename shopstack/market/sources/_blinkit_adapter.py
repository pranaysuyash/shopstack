...
def _find_json_file(data_dir: Path | None = None) -> Path:
    ...
    candidates = sorted(d.glob("blinkit_fresh_vegetables_*.json"))
    if not candidates:
        raise FileNotFoundError(
            f"No blinkit_fresh_vegetables_*.json found in {d}"
        )
    return candidates[-1]
...
def normalize_record(
    ...
    raw_name = str(raw.get("product_name", "")).strip()
    ...
    discount_amount = original_price - price if original_price > price else 0.0
    computed_discount = round(discount_amount / original_price * 100, 1) if original_price > 0 else 0.0
...
__all__ = ["BlinkitAdapter", "load_snapshot", "snapshot_freshness"]
